# resume-markdown-rendering Specification

## Purpose
Define how resume Markdown content is rendered consistently and safely across resume editing, preview, detail viewing, and PDF export scenarios.

## Requirements

### Requirement: 简历内容必须按 Markdown 语义渲染
系统 MUST 在简历版本编辑页和详情页中将简历内容按 Markdown 语义解析并渲染，而不是直接展示原始 Markdown 标记文本。
...
系统 MUST 在简历版本编辑页提供清晰的编辑模式和预览模式切换能力，交互上接近 VSCode 的 Markdown 使用方式。
...
系统 MUST 让简历编辑页预览、简历详情页预览和 PDF 导出基于一致的 Markdown 解析结果，避免不同场景展示不同内容结构。
...
系统 MUST 在将 Markdown 渲染结果注入页面或导出容器前进行安全控制，并按纯 Markdown 策略处理内容，不保留原生 HTML 标签。
...
系统 MUST 为常见 Markdown 元素提供适合简历阅读和 PDF 导出的排版样式，并覆盖 Markdown 表格。
